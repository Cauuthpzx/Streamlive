...
const roomCleanupTimer = setInterval(() => {
    ...
    roomList.forEach((room, roomId) => {
        if (room.getPeers().size === 0 && room.lastActivity && (now - room.lastActivity > ROOM_IDLE_TIMEOUT)) {
            log.debug('Removing idle room', { roomId });
            room.close();
...
function checkSocketRateLimit(socketId, event, maxPerMinute = 10) {
    const key = `${socketId}:${event}`;
    const now = Date.now();
    const record = socketRateLimits.get(key) || { count: 0, resetAt: now + 60000 };
    if (now > record.resetAt) {
        record.count = 0;
...
function startServer() {
    ...
    app.use(
        helmet({
            contentSecurityPolicy: false, // CSP needs custom config for WebRTC
            crossOriginEmbedderPolicy: false, // Needed for cross-origin media
    ...
    */
    ...
    app.post('/login', loginLimiter, async (req, res) => {
        ...
        if (!username || !password) {
            log.warn('Login failed: missing username or password', { username: req.body?.username });
                ...
                req.on('data', (chunk) => {
                    ...
                    if (receivedBytes > recMaxFileSize) {
                        ...
                        try {
                            ...
                            log.warn(`[Upload] Failed to cleanup partial file ${filePath}`, e.message);
                        }
    ...
    server.listen(config?.server?.listen?.port || 3010, () => {
        ...
        // Check for default secrets and warn if any are still in use
        const defaultSecrets = {
            JWT_SECRET: { value: jwtCfg.JWT_KEY, default: 'mirotalksfu_jwt_secret' },
            RTMP_SECRET: { value: rtmpCfg?.secret, default: 'mirotalkRtmpSecret' },
            RTMP_API_SECRET: { value: rtmpCfg?.apiSecret, default: 'mirotalkRtmpApiSecret' },
            API_KEY_SECRET: { value: config?.api?.keySecret, default: 'mirotalksfu_default_secret' },
        };
        const insecureDefaults = Object.entries(defaultSecrets)
            .filter(([, cfg]) => cfg.value === cfg.default)
            .map(([name]) => name);
        if (insecureDefaults.length > 0) {
            log.warn('WARNING: Default secrets detected! Change these before deploying to production', {
    ...
    io.on('connection', (socket) => {
        ...
        socket.on('getChatGPT', async ({ time, room, name, prompt, context }, cb) => {
            ...
            if (!checkSocketRateLimit(socket.id, 'getChatGPT')) {
                return cb({ message: 'Rate limit exceeded. Please wait before sending another request.' });
            }
        ...
        socket.on('getDeepSeek', async ({ time, room, name, prompt, context }, cb) => {
            ...
            if (!checkSocketRateLimit(socket.id, 'getDeepSeek')) {
                return cb({ message: 'Rate limit exceeded. Please wait before sending another request.' });
            }
    ...
    async function isValidToken(token) {
        return new Promise((resolve, reject) => {
            jwt.verify(token, jwtCfg.JWT_KEY, { algorithms: ['HS256'] }, (err, decoded) => {
                if (err) {
                    // Token is invalid
                    resolve(false);
                } else {
    ...
    function decodeToken(jwtToken) {
        ...
        const decodedToken = jwt.verify(jwtToken, jwtCfg.JWT_KEY, { algorithms: ['HS256'] });
    ...
    try {
        ...
        for (const [roomId, room] of roomList.entries()) {
            try {
                ...
                // Clean up presenters for this room
                delete presenters[roomId];
            } catch (err) {
